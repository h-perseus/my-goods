...
export const createApiConnectionService = () => {
  const list = async (searchOptions?: any): Promise<Connection[]> => {
  ...
  const get = async (id: string): Promise<Connection> => {
    const { data } = await AXIOS.get<Connection>(API_URLS.CONNECTION(id));

    return data;
  };

  const edit = async (id: string, body: Partial<Connection>): Promise<void> => {
