...
export const ConnectionsComponent = (): JSX.Element => {
  ...
  const handleDeleteConnections = useCallback(() => {
    deleteConnection(selectedConnections).then(() => {
      load();
    }).catch( e => {})
  }, [selectedConnections])
    ...
    <>
      ...
      >
        <Flex gap={"size-100"} marginBottom={'size-200'} justifyContent={'end'}>
          <Button isDisabled={selectedConnections.length === 0} variant="secondary" UNSAFE_style={{ borderRadius: '4px'}} onPress={handleDeleteConnections}>삭제</Button>
        </Flex>
        <View flex={1} minHeight={0} position={"relative"} overflow={'auto'}>
          {
            <ConnectionList connections={connections} setSelectedConnections={setSelectedConnections} handleDeleteItem={handleDeleteItem}></ConnectionList>
          }
        </View>
      </Flex>
    </>
