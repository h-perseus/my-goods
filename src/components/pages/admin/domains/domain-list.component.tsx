...
import { PATHS, ROUTER_PATHS } from "../../../../routes";
  ...
  export const DomainList = ({domains, setSelectedDomains, handleDeleteItem}: { domains: any[], setSelectedDomains: (ids: string[]) => void, handleDeleteItem: (id: string) => void}): JSX.Element => {
                ...
                if (key === "action") {
                    ...
                    <Cell>
                        <ActionButton isQuiet onPress={() => {
                          navigate(ROUTER_PATHS.ADMIN + PATHS.getDomainEditUrl(item._id))
                        }}>
                            <Edit />
                        </ActionButton>
                        <ActionButton isQuiet onPress={() => {
